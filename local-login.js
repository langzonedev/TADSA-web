...
export async function requireLocalSession(container,refresh,isCurrent=()=>true,hasDrafts=()=>false){
 ...
 if(status.authenticated){
  ...
  const signout=node('button','Sign out','secondary');signout.type='button';signout.id='local-signout';signout.onclick=async()=>{if(hasDrafts()&&!confirm('Sign out and discard unsaved changes?'))return;signout.disabled=true;try{await accountRequest('auth/logout',{});sessionEnded=true;lockWorkspace();container.replaceChildren();location.reload();}catch{signout.disabled=false;signout.textContent='Retry sign out';}};bar.append(signout);document.querySelector('#sidebar-session').append(bar);
