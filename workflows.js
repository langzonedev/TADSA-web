...
function coordinationFields(form, d, directories, focus) {
  ...
  function drawTechnicians() {
    ...
    for (const t of rows) {
      ...
      if (t.id) text.append(el('small', `${t.skills.join(' · ')}. ${t.openCount} open projects. Remaining work: ${t.remainingMinutes==null?'Not recorded':Math.round(t.remainingMinutes/60*100)/100+' hours'+(t.remainingWorkUnrecordedCount?' recorded; '+t.remainingWorkUnrecordedCount+' project'+(t.remainingWorkUnrecordedCount===1?'':'s')+' not estimated':' remaining')}. General allocation status: ${t.availability}. Base: ${[t.suburb,t.postcode].filter(Boolean).join(' ')||'Not recorded'}. Service areas: ${(t.serviceAreas||[]).map(a=>({western:'Western suburbs',southern:'Southern suburbs',northern:'Northern suburbs',eastern:'Eastern suburbs','adelaide-hills':'Adelaide Hills',metro:'Metropolitan Adelaide',regional:'Regional South Australia'}[a]||a)).join(', ')||'Not recorded'}.`));
