...
test('multi technician membership, assignments, workload, numbering and category survive reload',()=>{
 let d=normalise(seed());let run=(path,method='GET',input={})=>dispatch(d,path,method,input,new URLSearchParams(),admin);
...
test('restricted credentials denied including replay and audit redacted',()=>{
 const d=normalise(seed()),tech=d.people.find(p=>p.roles.includes('Technician')),path='people/'+tech.id+'/credentials';
...
test('demo password upgrade revokes legacy default session while preserving custom passwords',async()=>{
 memory.clear();const d=normalise(seed()),a=initialDemoAccounts();applyDemoMigration(a,await prepareDemoMigration(a));
