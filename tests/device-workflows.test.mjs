...
test('create/edit person, complete intake, project coordination and safe local deletion',()=>{
 ...
 let p=call('people','POST',{requestId:req(),name:'Fictional Tester',email:'tester@example.invalid',phone:'',roles:['Client','Technician'],technician:{skills:['Assessment'],availability:'available'},allowDuplicate:false});
 ...
 const project=call('projects','POST',{requestId:req(),clientId:c.id,primaryRecordId:null,title:'Synthetic assessment',summary:'Check posture',kind:'Assessment',requiredSkills:['Assessment'],technicianId:p.id,fundingStatus:'self',payerId:null,fundingNotes:''});assert.equal(project.clientName,p.name);
...
test('calendar/no-op areas and shared NDIS versions retain contract',()=>{
 ...
 const plan=call('projects/project-1/work-plan'),body={requestId:req(),version:plan.version,clientNdisVersion:0,location:'Synthetic location',serviceArea:'metro',ndisApplicable:true,ndisNumber:'123 456 789',ndisApproved:true,ndisApprovedBy:'Demo coordinator',ndisApprovedOn:'2026-09-18'};const saved=call('projects/project-1/work-plan','PUT',body);assert.equal(saved.clientNdisVersion,1);assert.equal(call('clients/client-1').ndis.number,'123 456 789');assert.equal(call('projects/project-1/work-plan','PUT',{...body,requestId:req(),version:saved.version,clientNdisVersion:1}).version,saved.version);
});
