...
test('calendar/no-op areas and shared NDIS versions retain contract',()=>{
 ...
 const entry=call('availability/person-3/entries','POST',{requestId:req(),id:null,version:0,startDate:'2026-10-01',endDate:'2026-10-02',status:'available',note:''});assert.equal(entry.version,2);assert.equal(entry.entries.length,1);const replacement=call('availability/person-3/entries','POST',{requestId:req(),id:null,version:0,calendarVersion:entry.version,startDate:'2026-10-02',endDate:'2026-10-03',status:'limited',note:'New range'});assert.equal(replacement.version,3);assert.deepEqual(replacement.entries.map(e=>[e.startDate,e.endDate,e.status]),[['2026-10-01','2026-10-01','available'],['2026-10-02','2026-10-03','limited']]);assert.equal(call('availability/person-3/areas','PUT',{requestId:req(),version:replacement.version,serviceAreas:['metro']}).version,replacement.version);
