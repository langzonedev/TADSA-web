...
export function demoAccountDispatch(accounts,data,path,method,input){
 ...
 const displayName=validText(input.displayName,120);if(!['administrator','coordinator'].includes(input.role))fail('Choose an account permission.');if(!data.people.some(p=>p.id===input.personId))fail('Choose an existing person.');
 let result;if(path==='accounts'&&method==='POST'){const username=validText(input.username,60).toLowerCase();if(!/^[a-z0-9][a-z0-9._-]{2,59}$/.test(username))fail('Use a 3–60 character username.');if(accounts.items.some(a=>a.username===username))fail('Username already exists.',409);if(accounts.items.some(a=>a.personId===input.personId))fail('This person already has a demo profile.',409);result={id:crypto.randomUUID(),username,displayName,personId:input.personId,role:input.role,active:true,version:1,mustChangePassword:false};accounts.items.push(result);}
 else if(parts.length===2&&method==='PUT'){const account=accounts.items.find(a=>a.id===parts[1]);if(!account)fail('Account not found.',404);if(account.version!==input.version)fail('Account changed. Reload before saving.',409);if(typeof input.active!=='boolean')fail('Set the active status.');if(account.role==='administrator'&&account.active&&(!input.active||input.role!=='administrator')&&accounts.items.filter(a=>a.role==='administrator'&&a.active).length===1)fail('Keep at least one active demo administrator.');if(accounts.items.some(a=>a.id!==account.id&&a.personId===input.personId))fail('This person already has a demo profile.',409);Object.assign(account,{displayName,personId:input.personId,role:input.role,active:input.active,version:account.version+1});result=account;}
