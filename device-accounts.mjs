...
export const DEMO_PASSWORD='TadsaDemo2026!';
...
const publicAccount=account=>Object.fromEntries(['id','username','displayName','personId','role','active','version','mustChangePassword'].map(k=>[k,account[k]]));
...
export async function prepareDemoMigration(accounts){const credentials={};for(const account of accounts.items)if(!account.credential)credentials[account.id]=await verifier(DEMO_PASSWORD);return credentials;}
export function applyDemoMigration(accounts,credentials){for(const account of accounts.items){account.personId=null;if(!account.credential&&credentials[account.id]){account.credential=credentials[account.id];account.credentialVersion=1;account.usesPublicDemoPassword=true;}}}
...
export function demoAccountDispatch(accounts,data,path,method,input,prepared={}){
 ...
 if(path==='auth/status'){const defaults=accounts.items.find(a=>a.username==='demo-admin'&&a.active&&a.usesPublicDemoPassword);return {configured:true,authenticated:!!user,user,mode:'device-demo',demoDefaults:defaults?{username:defaults.username,password:DEMO_PASSWORD}:null,demoMigrationHint:accounts.items.some(a=>a.username!=='demo-admin'&&a.usesPublicDemoPassword)?'Previously password-free demo accounts use TadsaDemo2026! until an administrator resets them.':null};}
 ...
 exact(resetting?['requestId','version','password']:method==='POST'?['requestId','username','displayName','role','password']:['requestId','version','displayName','role','active']);
 ...
 if(path==='accounts'&&method==='POST'){const username=validText(input.username,60).toLowerCase();if(!/^[a-z0-9][a-z0-9._-]{2,59}$/.test(username))fail('Use a 3–60 character username.');if(accounts.items.some(a=>a.username===username))fail('Username already exists.',409);if(!prepared.credential)fail('Password request was not prepared.');const account={id:crypto.randomUUID(),username,displayName,personId:null,role:input.role,active:true,version:1,mustChangePassword:false,credential:prepared.credential,credentialVersion:1,usesPublicDemoPassword:false};accounts.items.push(account);result=publicAccount(account);}
 else if(parts.length===2&&method==='PUT'){const account=accounts.items.find(a=>a.id===parts[1]);if(!account)fail('Account not found.',404);if(account.version!==input.version)fail('Account changed. Reload before saving.',409);if(typeof input.active!=='boolean')fail('Set the active status.');if(account.role==='administrator'&&account.active&&(!input.active||input.role!=='administrator')&&accounts.items.filter(a=>a.role==='administrator'&&a.active).length===1)fail('Keep at least one active demo administrator.');if(account.active!==input.active)account.credentialVersion++;Object.assign(account,{displayName,personId:null,role:input.role,active:input.active,version:account.version+1});result=publicAccount(account);}
