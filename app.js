...
async function project(view, id) {
  ...
  if(!destinations.includes(tab)&&!['Task','Edit details','Audit trail','Case-note history','Hold or resume work','Record hours','Prepare invoices','Print documents','Team & funding'].includes(tab)&&!tab.startsWith('Action:'))tab='Overview';
  view.classList.add('project-workspace');
  view.append(link('← All projects','projects','breadcrumb'));
  const meta=el('div',undefined,'record-meta'),header=recordHeader(view,p.reference,p.title,meta),headerActions=el('div',undefined,'project-more-wrap');header.append(headerActions);
  ...
  const content=el('div'),auditPane=el('section'),caseNotesPane=el('section');editPane.append(content);auditPane.append(button('← Project overview',()=>navigate('Overview'),'secondary'),auditPanel(p));caseNotesPane.append(button('← Project overview',()=>navigate('Overview'),'secondary'));auditPane.querySelector('h2').textContent='Audit trail';
  ...
  const actionSource=el('div'),care=el('div');
  const [projectOperations,projectLocation]=await Promise.all([api(`projects/${id}/operations`),api(`projects/${id}/location`)]);
  ...
  for(const child of [...care.children]){const name=child.querySelector(':scope > summary')?.textContent;if(child.classList.contains('project-files'))filesPane.append(child);else if(name==='Work location'||name==='NDIS reference')editPane.append(child);else costsPane.append(child);}
  flatten(filesPane);flatten(costsPane);flatten(recordPane);flatten(editPane);
  if(!recordPane.children.length){const empty=panel('Workflow history');empty.append(el('p','No workflow decisions have been recorded for this project yet. Existing case notes are available on Overview; technical changes remain under More → Audit trail.'));recordPane.append(empty);}
  filesPane.prepend(button('← Return to current task',()=>navigate('Task'),'secondary project-task-return'));
  costsPane.prepend(el('h2','Costs & invoices'));
  ...
  const state=lifecycle.state,stage=state?.stage,info=stages[stage]??['Review the project','This existing project retains its records. Begin the guided workflow when ready.','Review workflow'];
  const technician=p.relationships?.find(r=>r.id===p.coordination?.technicianId);
  const layout=el('div',undefined,'project-home-grid'),left=el('div'),notes=el('aside',undefined,'project-home-notes');layout.append(left,notes);overview.append(layout);
  ...
  const embedded=new Map([['Hold or resume work',{route:'operations',focus:'holds'}],['Record hours',{route:'operations',focus:'hours'}],['Prepare invoices',{route:'operations',focus:'invoices'}],['Print documents',{route:'documents'}],['Team & funding',{route:'coordination'}]].map(([name,config])=>{const pane=el('section',undefined,'project-embedded-pane');pane.hidden=true;pane.tabIndex=-1;view.append(pane);return [name,{...config,pane,loaded:false,loading:null}];}));
  async function loadEmbedded(name){const entry=embedded.get(name);if(!entry||entry.loaded)return;if(entry.loading)return entry.loading;entry.pane.replaceChildren(el('p','Opening '+name.toLowerCase()+'…','loading'));entry.loading=(async()=>{const body=el('div');try{if(entry.route==='coordination')await renderWorkflow(body,'coordination',id,{embedded:true,onCancel:()=>{entry.loaded=false;navigate('Overview');}});else await renderOperations(body,entry.route,id,{embedded:true,focus:entry.focus});entry.pane.replaceChildren(el('h2',name),body);entry.loaded=true;}catch(error){entry.pane.replaceChildren(message(error.status?error.message:'This section could not load. Your project is still open.',true),button('Try again',()=>loadEmbedded(name),'secondary'));}finally{entry.loading=null;}})();return entry.loading;}
  const more=button('More ⋯',()=>{menu.hidden=!menu.hidden;more.setAttribute('aria-expanded',String(!menu.hidden));if(!menu.hidden)menu.querySelector('button,a')?.focus();},'secondary'),menu=el('div',undefined,'project-more-menu');menu.hidden=true;more.setAttribute('aria-expanded','false');more.setAttribute('aria-controls','project-more-menu');menu.id='project-more-menu';
  menu.append(button('Audit trail',()=>navigate('Audit trail'),''),button('Case-note history',()=>navigate('Case-note history'),''));
  menu.append(button('Edit project details',()=>{editMode=false;navigate('Edit details');},''));for(const [key,value]of actionPanes)menu.append(button(value.title,()=>navigate(key),''));menu.append(button('Team & funding',()=>navigate('Team & funding'),''),button('Record hours',()=>navigate('Record hours'),''),button('Hold or resume work',()=>navigate('Hold or resume work'),''),button('Print documents',()=>navigate('Print documents'),''));headerActions.append(more,menu);headerActions.addEventListener('keydown',e=>{if(e.key==='Escape'){menu.hidden=true;more.setAttribute('aria-expanded','false');more.focus();}});
  view.addEventListener('click',event=>{const anchor=event.target.closest('a[href]');if(anchor&&!event.ctrlKey&&!event.metaKey&&!event.shiftKey&&!event.altKey&&event.button===0){const [route,target,focus]=anchor.hash.slice(1).split('/');if(target===id){const section=route==='coordination'?'Team & funding':route==='documents'?'Print documents':route==='operations'?({hours:'Record hours',holds:'Hold or resume work',invoices:'Prepare invoices'})[focus]??'Prepare invoices':null;if(section){event.preventDefault();navigate(section);}}}if(!headerActions.contains(event.target)){menu.hidden=true;more.setAttribute('aria-expanded','false');}});
  ...
  function draw(){
    ...
    for(const pane of [task,editPane,auditPane,caseNotesPane,...[...embedded.values()].map(e=>e.pane),...[...actionPanes.values()].map(a=>a.pane)]){pane.removeAttribute('id');pane.removeAttribute('role');pane.removeAttribute('aria-labelledby');}for(const [name,entry]of embedded)entry.pane.hidden=tab!==name;const focused=embedded.get(tab)?.pane??(tab==='Task'?task:tab==='Edit details'?editPane:tab==='Audit trail'?auditPane:tab==='Case-note history'?caseNotesPane:actionPanes.get(tab)?.pane);if(focused){focused.id='project-focused-pane';focused.setAttribute('role','tabpanel');if(embedded.has(tab)){focused.removeAttribute('aria-labelledby');focused.setAttribute('aria-label',tab);}else focused.setAttribute('aria-labelledby','project-tab-0');}task.hidden=tab!=='Task';editPane.hidden=tab!=='Edit details';auditPane.hidden=tab!=='Audit trail';caseNotesPane.hidden=tab!=='Case-note history';auditPane.tabIndex=caseNotesPane.tabIndex=-1;task.tabIndex=-1;editPane.tabIndex=-1;for(const [key,{pane}]of actionPanes){pane.hidden=tab!==key;pane.tabIndex=-1;}
