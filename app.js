...
async function project(view, id) {
  ...
  const destinations=['Overview','Files','Costs & invoices','History'];
  if(!destinations.includes(tab)&&!['Task','Edit details'].includes(tab)&&!tab.startsWith('Action:'))tab='Overview';
  ...
  const content=el('div');editPane.append(content);
  ...
  view.append(overview,task,filesPane,costsPane,recordPane,editPane);
  ...
  if(state?.cancellation)warnings.unshift('Cancelled: '+state.cancellation.reason);if(state?.revisionRequested)warnings.unshift('Quote revision required: '+state.revisionRequested.reason);
  ...
  renderNotes(notes,id,projectOperations,true);const notesLink=notes.querySelector('a[href="#operations/'+id+'"]');if(notesLink)notesLink.replaceWith(button('View all case notes',()=>navigate('History'),'text-button'));
  ...
  function navigate(name){tab=name;menu.hidden=true;more.setAttribute('aria-expanded','false');draw();const destination=name==='Task'?task:name==='Overview'?overview:name==='Files'?filesPane:name==='Costs & invoices'?costsPane:name==='History'?recordPane:name==='Edit details'?editPane:actionPanes.get(name)?.pane;destination?.focus({preventScroll:true});tabs.scrollIntoView({block:'nearest'});}
  function draw(){
    ...
    for(const pane of [task,editPane,...[...actionPanes.values()].map(a=>a.pane)]){pane.removeAttribute('id');pane.removeAttribute('role');pane.removeAttribute('aria-labelledby');}const focused=tab==='Task'?task:tab==='Edit details'?editPane:actionPanes.get(tab)?.pane;if(focused){focused.id='project-focused-pane';focused.setAttribute('role','tabpanel');focused.setAttribute('aria-labelledby','project-tab-0');}task.hidden=tab!=='Task';editPane.hidden=tab!=='Edit details';task.tabIndex=-1;editPane.tabIndex=-1;for(const [key,{pane}]of actionPanes){pane.hidden=tab!==key;pane.tabIndex=-1;}
