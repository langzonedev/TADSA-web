...
async function project(view, id) {
  ...
  for(const [label,roles] of [['CONTACTS',['Carer','OT','Occupational therapist','Allied-health contact']],['REFERRER',['Referrer']],['FUNDER',['Funder']],['TECHNICAL TEAM',['Technician']]]){
  ...
  const money=value=>new Intl.NumberFormat('en-AU',{style:'currency',currency:'AUD'}).format(value/100),fullDate=value=>value?new Intl.DateTimeFormat('en-AU',{day:'numeric',month:'short',year:'numeric'}).format(new Date(value.slice(0,10)+'T12:00:00')):'Not recorded';
  ...
  layout.append(brief,finance,notes,left);
  ...
  for(const [i,[name]] of groups.entries()){const item=el('li',(i<phaseIndex?'✓ ':`${i+1} · `)+name);if(i===phaseIndex){item.setAttribute('aria-current','step');item.className='current';}else if(i<phaseIndex)item.className='complete';phase.append(item);}if(lifecycle.managed)left.prepend(phase);
  ...
  function draw(){
    projectTabs.set(id,tab);header.querySelector('h1').textContent=p.title;header.querySelector('.eyebrow').textContent='Project no. '+p.reference;financeFacts.lastElementChild.querySelector('dd').textContent=p.invoiceRequired?'Yes':'No';meta.replaceChildren();
