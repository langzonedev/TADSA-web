...
installModelExtensions(ctx=>{
 ...
 if(kind==='reports'&&method==='GET'){
  ...
  const management=buildManagementReport({projects:d.projects.map(p=>({...p,programme:d.projectAdminDetails?.[p.id]?.programme??'',actualMinutes:d.operations[p.id]?.actualMinutes,history:d.lifecycles?.[p.id]?.history??[]})),events:d.statusEvents.map(e=>({...e,projectId:e.id})),invoices:d.projects.flatMap(p=>(d.operations[p.id]?.invoices??[]).map(i=>({...i,projectId:p.id}))),feedback:Object.entries(d.projectFeedback??{}).flatMap(([projectId,entries])=>entries.map(e=>({...e,projectId}))),technicians:d.people.filter(p=>p.active!==false&&p.roles.includes('Technician'))},month);
  return {management,month,timeZone:'Australia/Adelaide',detailLimit:100,detailNotice:'Lists are limited to 100 records and activity lists use the latest 100 events. Totals include all matching records.',created:{count:unique(created).length,items:unique(recent.filter(x=>x.fromStatus===null)),unknownHistoricalCount:projectList.filter(p=>!knownCreated.has(p.id)).length},closed:{count:unique(closed).length,eventCount:closed.length,items:unique(recent.filter(x=>x.toStatus==='closed')),unknownHistoricalCount:projectList.filter(p=>p.status==='closed'&&!knownClosed.has(p.id)).length},reopened:{count:unique(reopened).length,eventCount:reopened.length,items:unique(recent.filter(x=>x.fromStatus==='closed'&&x.toStatus!=='closed'))},currentOpen:{count:open.length,items:open.slice(0,100)},statusCounts,workload,billing:{invoicedCents,receivedCents,invoices,payments}};
 }
});
