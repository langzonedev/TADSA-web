...
if (location.protocol === 'file:') {
  ...
  application.onerror=()=>{const host=document.querySelector('#auth-form-host');if(!host)return;host.hidden=false;document.querySelector('#auth-open').hidden=true;document.querySelector('#auth-status').textContent='';const message=document.createElement('p');message.textContent='The workspace could not load. Check the local service, then reload to try again.';const retry=document.createElement('button');retry.type='button';retry.textContent='Reload';retry.onclick=()=>location.reload();host.replaceChildren(message,retry);};
